...
// Function to fetch news for a specific year
function fetchNews(year) {
    // Dummy data for demonstration
    const newsData = getNewsDataForYear(year);

    // Clear previous news
    const newsContainer = document.querySelector(".news");
    newsContainer.innerHTML = "";

    // Display news for the selected year
    newsData.forEach((item) => {
        const newsItem = document.createElement("div");
        newsItem.classList.add("news-item");
        newsItem.innerHTML = `<strong>${item.date}</strong>: ${item.title}`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    yearSelector.addEventListener("change", function () {
        const selectedYear = this.value.match(/\d+/)[0]; // Extract the year from the selected value using regular expression
        fetchNews(selectedYear);
    });

    // Get the year from the URL and fetch corresponding news
    const currentYear = window.location.pathname.match(/\d{4}/);
    if (currentYear) {
        fetchNews(currentYear[0]);
    } else {
        // Default load - display news for 2024
        fetchNews("2024");
    }
});
